...
int main() {
    ...
    int curInput, lastBoard;
    set<int> wonBingos;
    bool exit = false;
    for (int i = 0; i < input.size() && !exit; ++i) {
        curInput = input[i];
        for (int j = 0; j < allBingos.size() && !exit; ++j) {
            allBingos[j].mark(curInput);
            if (allBingos[j].hasWon()) {
                wonBingos.insert(j);
            }

            if (wonBingos.size() == allBingos.size()) {
                exit = true;
                lastBoard = j;
            }
        }
    ...
    */
    ...
    allBingos[lastBoard].printBingo();

    cout << allBingos[lastBoard].calculateSum(curInput);
