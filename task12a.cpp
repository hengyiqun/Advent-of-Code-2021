...
void dfs(vector<vector<int>> &AL, vector<bool> visited, unordered_map<int, string> &umapRev, int curNode, int* numWays) {
    ...
    for (auto &n: AL[curNode]) {
        if (!visited[n] && umapRev[n] != "start") {
            dfs(AL, visited, umapRev, n, numWays);
        }
    }
}
...
int main() {
    ...
    dfs(AL, visited, umapRev, start, &numWays);
    cout << numWays;
    return 0;
}